...
function show_user_library() {
    ...
    $user_library_sql = 'SELECT games.ID as ID, LOGO_URL, TITLE, DESCRIPTION FROM games JOIN games_users ON games_users.GAME_ID=games.ID JOIN users ON users.ID=games_users.USER_ID WHERE users.ID=?';
